SUPERUSERS = {
    "superusers_menu": {
        "en": "👮Trusted users menu",
        "uk": "👮Меню довірених користувачів",
    },
    "add_superusers": {"en": "📥Add users", "uk": "📥Додати користувачів"},
    "delete_superusers": {
        "en": "🗑️Delete users from trusted",
        "uk": "🗑️Видалити користувачів з довірених",
    },
    "superusers_list": {
        "en": "📑Trusted users list",
        "uk": "📑Список довірених користувачів",
    },
    "no_superusers": {
        "en": "No trusted users were found.",
        "uk": "Жодного довіреного користувача не було знайдено.",
    },
    "ask_s_users_to_delete": {
        "en": "Please select the users you want to delete:",
        "uk": "Будь ласка оберіть користувачів котрих ви бажаєте видалити: ",
    },
    "not_found_users": {
        "en": "These users were not on this server and were deleted as well:",
        "uk": "Цих користувачів не було знайдено на сервері тому вони були видалені:",
    },
    "current_superusers": {
        "en": "Current trusted users:",
        "uk": "Поточні довірені користувачі:",
    },
    "superuser_names": {
        "en": "Trusted user names",
        "uk": "Імена довірених користувачів",
    },
    "ask_s_users_names": {
        "en": "Please type user names",
        "uk": "Будь ласка напишіть імена користувачів",
    },
    "added_superusers": {"en": "Added users", "uk": "Додані користувачі"},
    "not_found_for_addition": {
        "en": "Not found on the server",
        "uk": "Не знайдені на сервері",
    },
    "already_superusers": {
        "en": "Already trusted users",
        "uk": "Уже є довіреними користувачами",
    },
    "deleted_superusers": {"en": "Deleted users", "uk": "Видалені користувачі"},
}
