...
class SettingsFormatter:
    ...
    def format_current_main_settings(
        ...
        settings = self.settings.dict_storage.get_all(
            target=StorageTarget.SETTINGS, guild_id=guild.id
        )
    ...
    def _format_status(self, guild: discord.Guild, key: str, value) -> str:
        if key == "verification_role_id":
            status = self.translator.t(
                guild_id=guild.id, section="EDIT_SETTINGS", key="not_assigned"
            )
    ...
    async def format_current_system_channels(
        self, guild: discord.Guild
    ) -> discord.Embed:
        not_found_ch: list[str] = []

        channels = self.settings.dict_storage.get_all(
            target=StorageTarget.SYSTEM_CHANNELS, guild_id=guild.id
        )

        lines: list[str] = [
            f"System channels{' ' * 8}channel names",
            f"{'-' * 21}{' ' * 2}{'-' * 14}",
        ]
        ...
        for key, value in sorted(channels.items(), key=lambda item: item[0]):
            ...
            config_name = key.removesuffix("_id").replace("_", " ")
            lines.append(
                f"{config_name:<20}: {channel.name if channel else '❗ not assigned'}"
            )

        if not_found_ch:
            msg = await self.service.clean_up_system_channels(
                guild_id=guild.id, channels=not_found_ch
            )
            lines.append(f"\n\n{msg}")
    ...
    async def format_current_hidden(
        ...
        items = self.settings.set_storage.for_set_get(target=target, guild_id=guild.id)
        ...
        if not_found:
            message = await (
                self.service.clean_up_hidden_channels(
                    guild_id=guild.id, values=not_found
                )
                if is_channel
                else self.service.clean_up_hidden_roles(
                    guild_id=guild.id, role_ids=not_found
                )
            )
            lines.append(f"\n\n{message}")
