...
class DeleteMessageFlow:
    ...
    async def delete_message(
        ...
        if users_names:
            result = await self._delete_message_from_users(
                guild=guild, channel=channel, amount=amount, users_list=users_names
            )

            if not result:
                embed = ErrorEmbed(
                    description=self.translator.t(
                        guild_id=guild.id, section="SYSTEM_GENERAL", key="msg_not_found"
                    )
                )
            else:
    ...
    async def _delete_message_from_users(
        ...
        user_names = self.service.get_users(guild=guild, users=users_list)
    ...
    async def _delete_any_message_process(
        channel: discord.TextChannel, amount: int
    ) -> int:
