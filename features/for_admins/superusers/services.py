...
class SuperusersService(DBBaseService):
    ...
    async def _build_not_found_message(self, user_ids: set[int], client: discord.Client, guild_id: int) -> str:
    ...
    async def _build_cleaned_message(self, user_ids: set[int], client: discord.Client, guild_id: int) -> str:
        msg = self.translator.t(
            guild_id=guild_id,
            section='SUPERUSERS',
            key='not_found_users'
