...
class SuperusersFormatter:
    ...
    def build_embed(self, guild: discord.Guild) -> discord.Embed:
        msg = self.translator.t(
            guild_id=guild.id, section="SUPERUSERS", key="current_superusers"
        )
        ...
        if not users:
            ...
            return WarningEmbed(description=not_found)
        else:
    ...
    def build_add_result_embeds(
        self, result: AddSuperusersResult, guild_id: int
    ) -> list[discord.Embed]:
        embeds: list[discord.Embed] = []

        if result.added_names:
            added_title = self.translator.t(
                guild_id=guild_id, section="SUPERUSERS", key="added_superusers"
            )
            embeds.append(
                SuccessEmbed(
                    description=self._format_block(
                        title=added_title, items=result.added_names
                    )
                )
            )
        ...
        if result.not_found:
            not_found_title = self.translator.t(
                guild_id=guild_id, section="SUPERUSERS", key="not_found_for_addition"
            )
            embeds.append(
                ErrorEmbed(
                    description=self._format_block(
                        title=not_found_title, items=result.not_found
                    )
                )
            )
        ...
        if result.already_super:
            already_superusers_title = self.translator.t(
                guild_id=guild_id, section="SUPERUSERS", key="already_superusers"
            )
            embeds.append(
                WarningEmbed(
                    description=self._format_block(
                        title=already_superusers_title, items=result.already_super
                    )
                )
            )
    ...
    def build_delete_result_embeds(
        self, result: DeleteSuperusersResult, guild: discord.Guild
    ) -> list[discord.Embed]:
        deleted_superusers_title = self.translator.t(
            guild_id=guild.id, section="SUPERUSERS", key="deleted_superusers"
        )

        embeds: list[discord.Embed] = [
            SuccessEmbed(
                description=self._format_block(
                    title=deleted_superusers_title, items=result.deleted
                )
            )
        ]
