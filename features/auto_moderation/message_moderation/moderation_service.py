...
class ModerationService:
    ...
    async def process_message(self, message: discord.Message) -> None:
        ...
        data = self.settings.dict_storage.get_all(
            target=StorageTarget.SETTINGS, guild_id=message.guild.id
        )
        admins = self.settings.set_storage.for_set_get(
            target=StorageTarget.SUPERUSERS, guild_id=message.guild.id
        )
        ...
        if data.get("spam_checking", False):
            await self._check_spam(
                message=message, timestamp=timestamp, attachments=attachments
            )
    ...
    async def _check_spam(
        ...
        if attachments:
            result = await self.service.check_attachment_spam(
                message=message, timestamp=timestamp
            )
            if result.value:
                await self._timeout_for_spam(
                    messages=result.messages, guild=message.guild
                )
                return
    ...
    async def _timeout_for_flood(self, member: discord.Member) -> None:
        timeout_until = datetime.now(UTC) + timedelta(minutes=20)
        await member.edit(
            timed_out_until=timeout_until,
            reason=self.translator.t(
                guild_id=member.guild.id, section="MODERATION", key="flood"
            ),
        )
    ...
    async def _timeout_for_spam(
        ...
        timeout_until = datetime.now(UTC) + timedelta(hours=48)
            ...
            if allowed_ban:
                try:
                    await user.ban(
                        ...
                        reason=self.translator.t(
                            guild_id=guild_id, section="MODERATION", key="raid"
                        ),
                    )
        ...
        await self._send_notification(guild=guild, message="\n".join(messages_to_sent))
