...
class FinishingConfiguration:
    ...
    async def send_the_result(self, interaction: discord.Interaction) -> None:
        ...
        summary = (
            ...
            f"🔹 Member left message: {'Enabled' if self.parent.config.get('member_left_enabled') else 'Disabled'}\n"
            f"🔹 Block users: {'Enabled' if self.parent.config.get('block_users_enabled') else 'Disabled'}\n"
        ...
        await interaction.response.send_message(
            f'```Congratulations the bot is ready for usage.```\n\n{summary}',
            ephemeral=True
        )
