...
class ManagementCog(commands.Cog):
    ...
    async def management(self, ctx):
        super_users_data = self.db.get_data(
            ctx.guild.id,
            'super_users',
            'user_id'
        )
        ...
        super_users = [user['user_id'] for user in super_users_data]
        ...
        if not settings:
            await ctx.send('```Settings are not configured for this server!```', delete_after=120)
            return
